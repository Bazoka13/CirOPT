...
def abc_test():
    for b in bench_queue:
        ...
        pure_cnf = BENCH_DIR+tmpb+".aig"
        cnf_list = [pure_cnf]
        for cnf in cnf_list:
            opt_cnf = cnf.replace("/original/","/opt/")
            if os.path.exists(opt_cnf):
                watch_log = bench_and_log[1]
                initial_log = bench_and_log[2]
                delete_dir_if_exists(watch_log)
                delete_dir_if_exists(initial_log)
                execmd("./runsolver -C 10000 -w "+watch_log+" -o "+initial_log+" "+ABC_DIR+" -c '&read "+cnf+"; &cec "+opt_cnf+";quit;'")
            else:
                watch_log = bench_and_log[1]
                initial_log = bench_and_log[2]
                delete_dir_if_exists(watch_log)
                delete_dir_if_exists(initial_log)
                execmd("./runsolver -C 10000 -w "+watch_log+" -o "+initial_log+" "+ABC_DIR+" -c '&read "+cnf+"; &cec -m; quit;'")
...
f=os.listdir("../benchmarks/orignal/")
for bench in f:
    if not ".aig" in bench:
        continue
